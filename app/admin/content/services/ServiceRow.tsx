  ...
  return (
    <>
      ...
      <div ref={setNodeRef} style={style}
        className="flex items-center gap-3 pl-2 pr-3 lg:px-5 py-2.5 hover:bg-[#fbf8ef]/50 transition-colors">
        {/* Drag handle */}
        ...
        <img src={service.image || getAutoImage(service.title)} alt={service.title}
          className="w-12 h-12 rounded-lg object-cover border border-[#dad5cd] shrink-0 hidden sm:block" />
        {/* Title + description */}
        <div className="flex-1 min-w-0">
          <p className="font-heading text-[#251d1b] text-[14px] font-bold truncate">{service.title}</p>
          <p className="font-body text-[#746e6b] text-[12px] line-clamp-1 mt-0.5 hidden sm:block">{service.description}</p>
        </div>
        {/* Toggle */}
        <button
          onClick={() => toggleService(service.id, !service.active)}
          className={`inline-flex items-center justify-center w-11 h-6 rounded-full transition-colors shrink-0 ${service.active ? "bg-[#44312b]" : "bg-[#dad5cd]"}`}
